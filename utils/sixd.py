...
def load_sixd(base_path, sequences, load_mesh=True):
    ...
    if load_mesh:
        for sequence in sequences:
            ...
            model.load(os.path.join(base_path, 'models/' + name + '.ply'), scale=bench.scale_to_meters)
            fps = FPS(model.vertices, 12)
            fps.fit()
            model.fps = np.concatenate([fps.selected_points, np.ones((fps.n_selected_pts, 1), dtype=float)], axis=1)
